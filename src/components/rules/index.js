...
const extraCodeRegExp = new RegExp(
    /^[a-zA-Z\d]+((_[a-zA-Z\d]+)*$|(:[a-zA-Z\d]+)*$|(-[a-zA-Z\d]+)*$)/
);
//外链URL正则
// eslint-disable-next-line
const remoteUrlRegExp = new RegExp(
    /^https?:\/\/(www.)?(?!www)[\da-z]+(-[\da-z])?(\.[a-z]+)*(\.[a-z]{2,5})((\/\w+(-\w+)?)|(\/:\w+))*\/?(\?([^=?&]+=([^=?&]+)?&?)*)?$/
);
// 内联URL正则
...
//手机号
const phoneRegExp = new RegExp(/^1[3|4|5|6|7|8][0-9]\d{8}$/);
/**
 ...
 * @param {*} value
 */
export const matchPhone = fileType => {
    return phoneRegExp.test(fileType);
};
...
/* export const matchEmoji = value => {
    return /\p{Emoji_Modifier_Base}\p{Emoji_Modifier}?|\p{Emoji_Presentation}|\p{Emoji}\uFE0F/gu.test(
        value
    );
};
 */
 ...
 * @param {*} value
 */
export const matchPhoneNumber = value => /1[3456789]\d{9}/.test(value);

/**
 * 匹配空格
 * @param {*} value
 */
export const matchBlankSpace = value => /\s/g.test(value);

/**
 * 匹配两端有空格
 * @param {*} value
 */
export const matchBothBlankSpace = value => /^[^ ]+[\s\S]*[^ ]+$/.test(value);
